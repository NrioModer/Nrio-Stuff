...
extern MY_HIGHP_OR_MEDIUMP vec2 vivid;
...
vec3 round_to_palette(vec3 color) {
    const vec3[15] palette = vec3[15](
        vec3(0.0,     0.0,     0.0),     // #000000
        vec3(1.0,     1.0,     1.0),     // #ffffff
        vec3(1.0,     0.0,     0.3020),  // #ff004d
        vec3(1.0,     0.6392,  0.0),     // #ffa300
        vec3(1.0,     0.9255,  0.1529),  // #ffec27
        vec3(0.0,     0.8941,  0.2118),  // #00e436
        vec3(0.1608,  0.6784,  1.0),     // #29adff
        vec3(0.5137,  0.4627,  0.6118),  // #83769c
        vec3(1.0,     0.4667,  0.6588),  // #ff77a8
	vec3(0.1137,  0.1686,  0.3255),  // #1d2b53
        vec3(0.4941,  0.1451,  0.3255),  // #7e2553
        vec3(0.0,     0.5294,  0.3176),  // #008751
        vec3(0.6706,  0.3216,  0.2118),  // #ab5236
        vec3(0.3725,  0.3412,  0.3098),  // #5f574f
        vec3(1.0,     0.8,     0.6667)   // #ffccaa
    );
    ...
    for(int i = 0; i < 15; i++) {
        float dist = 1.0 * abs(color.r - palette[i].r) 
                   + 11.0 * abs(color.g - palette[i].g) 
                   + 1.0 * abs(color.b - palette[i].b);      
        if(dist < min_dist) {
            min_dist = dist;
            index = i;
        }
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    if(vivid.g != 0.0) hsl.z = 1.0 - hsl.z;
    if(vivid.g != 0.0) hsl.z = 1.0 - hsl.z;
