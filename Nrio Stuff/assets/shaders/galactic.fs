...
vec3 round_to_palette(vec3 color) {
    const vec3[18] palette = vec3[18](
        ...
        vec3(0.3373, 0.4549, 0.4706),  
        vec3(0.3608, 0.5216, 0.5373),  
        vec3(0.3922, 0.6078, 0.6667),  
        vec3(0.4353, 0.6627, 0.7020),  
        vec3(0.4784, 0.7176, 0.7608), 
        vec3(0.5176, 0.7725, 0.8235),  
        vec3(0.5882, 0.8157, 0.8627),   
        vec3(0.6588, 0.8588, 0.9020),  
        vec3(0.7294, 0.9020, 0.9412),  
        vec3(0.8000, 0.9451, 0.9804),   
        vec3(0.8431, 0.9608, 0.9882),   
        vec3(0.8824, 0.9725, 0.9922),  
        vec3(0.9176, 0.9843, 0.9961), 
        vec3(0.9451, 0.9922, 0.9980),   
        vec3(0.9725, 0.9961, 0.9990),  
        vec3(0.9882, 0.9980, 0.9995),  
    ...
    for(int i = 0; i < 18; i++) {
        float dist = 0.1*abs(color.r - palette[i].r) 
           + 0.7*abs(color.g - palette[i].g) 
           + 0.5*abs(color.b - palette[i].b);
        if(dist < min_dist) {
            min_dist = dist;
            index = i;
        }
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    if(galactic.g != 0.0) hsl.z = 1.0 - hsl.z;
    if(galactic.g != 0.0) hsl.z = 1.0 - hsl.z;
