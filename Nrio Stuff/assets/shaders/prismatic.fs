...
vec3 round_to_palette(vec3 color) {
    const vec3[18] palette = vec3[18](
        vec3(0.3098, 0.3882, 0.4039),  
        vec3(0.4275, 0.4471, 0.3608),  
        vec3(0.5451, 0.5059, 0.3176),  
        vec3(0.7804, 0.6980, 0.2902), 
        vec3(0.3686, 0.4471, 0.5922),  
        vec3(0.4392, 0.5373, 0.6902),  
        vec3(0.5098, 0.6275, 0.7882), 
        vec3(0.3882, 0.5608, 0.8824), 
        vec3(0.5882, 0.6667, 0.7961),
        vec3(0.6706, 0.7098, 0.8275),  
        vec3(0.7529, 0.7529, 0.8588),  
        vec3(0.7490, 0.7804, 0.8353), 
        vec3(0.8235, 0.8431, 0.8863), 
        vec3(0.8980, 0.9059, 0.9373), 
        vec3(0.9451, 0.9490, 0.9686), 
        vec3(0.9725, 0.9765, 0.9843),  
        vec3(0.9882, 0.9922, 0.9961),  
    ...
    for(int i = 0; i < 18; i++) {
        float dist = 0.40*abs(color.r - palette[i].r) 
           + 0.78*abs(color.g - palette[i].g) 
           + 0.63*abs(color.b - palette[i].b);
        if(dist < min_dist) {
            min_dist = dist;
            index = i;
        }
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    if(prismatic.g != 0.0) hsl.z = 1.0 - hsl.z;
    if(prismatic.g != 0.0) hsl.z = 1.0 - hsl.z;
